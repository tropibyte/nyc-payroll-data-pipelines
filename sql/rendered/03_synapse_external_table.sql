...
IF NOT EXISTS (SELECT * FROM sys.external_data_sources WHERE name = 'ds_nycpayroll_staging')
    ...
    WITH ( LOCATION = 'abfss://adlsnycpayroll-tarie-n@adlsnycpayrolltarien.dfs.core.windows.net' );
GO
...
/* ---------------------------------------------------------------------
   ...
   Generate the SAS with:
     az storage container generate-sas --account-name adlsnycpayrolltarien \
        --name adlsnycpayroll-tarie-n --permissions rl --expiry <yyyy-mm-dd> \
